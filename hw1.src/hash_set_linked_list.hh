...
public:
    ...
    void insert(const T& item){
        ...
        if(m_num_elements > m_impl.bucket_number() / GROW_FACTOR){
...
private:
    ...
    class impl {
    public:
        explicit impl(size_t bucket_number) : m_bucket_number{bucket_number} {
            m_buckets.assign(m_bucket_number, {});
        }
        ...
        size_t bucket_number(){
            return m_bucket_number;
        }

        /// Creates impl with same items but bigger bucket count
        impl create_bigger_self(){
            impl new_impl{static_cast<size_t>(GROW_FACTOR * m_bucket_number)};
